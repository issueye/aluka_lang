...
func NewNavigator(ctx engine.Context, cfg NavigatorConfig) error {
	_, navProto, err := gbase.RegisterInterface(ctx, gbase.WebInterface{Name: "Navigator"})
	if err != nil {
		return err
	}
...
func NewBroadcastChannel(ctx engine.Context, cfg gevent.MessageConfig) error {
...
func deliverBroadcast(state *bcState, msg engine.Value) {
	ev := gevent.NewMessageEventInstance([]engine.Value{engine.Str("message")})
