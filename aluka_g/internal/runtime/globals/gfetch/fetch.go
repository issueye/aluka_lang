...
func buildResponse(ctx engine.Context, args []engine.Value, status int, statusText string, headerArg engine.Value, statusSet bool) engine.Value {
	...
	if bodyStr != "" {
		bodyStream, _ := gstream.NewReadableStream(ctx, []engine.Value{engine.NewObjectFrom(map[string]engine.Value{
					...
					if c, ok := a[0].AsObject(); ok {
						if e, err := c.Get("enqueue"); err == nil && e.IsFunction() {
							if f, ok := e.AsFunction(); ok {
								if _, err := f.Call([]engine.Value{gbuffer.NewBufferInstance([]byte(bodyStr))}); err != nil {
									interpreter.ReportUncaught(nil, err)
								}
							}
						}
	...
	_ = res.Set("text", engine.NewFunction("text", func(a []engine.Value) (engine.Value, error) {
		...
		return gbase.ResolveValue(ctx, engine.Str(bodyStr))
	}))
	// arrayBuffer() → Promise<Buffer>
	_ = res.Set("arrayBuffer", engine.NewFunction("arrayBuffer", func(a []engine.Value) (engine.Value, error) {
		_ = res.Set("bodyUsed", engine.Boolean(true))
		return gbase.ResolveValue(ctx, gbuffer.NewBufferInstance([]byte(bodyStr)))
	}))
	// json() → Promise<parsed>
	_ = res.Set("json", engine.NewFunction("json", func(a []engine.Value) (engine.Value, error) {
		...
		if err != nil || !parseFn.IsFunction() {
			return gbase.RejectValue(ctx, "JSON.parse not available")
		}
...
func newStreamingFetchResponse(ctx engine.Context, status int, statusText string, headers engine.Value, url string) (engine.Value, *fetchBodyState) {
	...
	streamValue, _ := gstream.NewReadableStream(ctx, nil)
...
func (b *fetchBodyState) consume(kind string) (engine.Value, error) {
	if !b.markUsed() {
		return gbase.RejectValue(b.ctx, "Body has already been consumed")
	}
	return gbase.NewPromise(b.ctx, engine.NewFunction("executor", func(args []engine.Value) (engine.Value, error) {
		if len(args) < 2 {
			return engine.Undefined(), nil
		}
...
func (b *fetchBodyState) append(chunk []byte) {
	...
	if enqueue, err := b.stream.Get("enqueue"); err == nil && enqueue.IsFunction() {
		if f, ok := enqueue.AsFunction(); ok {
			if _, err := f.Call([]engine.Value{gbuffer.NewBufferInstance(chunk)}); err != nil {
				interpreter.ReportUncaught(nil, err)
			}
		}
	}
...
func (b *fetchBodyState) settle(waiter fetchBodyWaiter) {
	if b.err != "" {
		gbase.CallRejectError(waiter.reject, gbase.NewTypeError(b.ctx, b.err))
		return
	}
	switch waiter.kind {
	case "arrayBuffer":
		gbase.CallResolve(waiter.resolve, gbuffer.NewBufferInstance(append([]byte(nil), b.data...)))
	case "json":
		jsonGlobal, err := b.ctx.Global().Get("JSON")
		if err != nil || !jsonGlobal.IsObject() {
			gbase.CallRejectError(waiter.reject, gbase.NewTypeError(b.ctx, "JSON not available"))
			return
		}
		jsonObject, _ := jsonGlobal.AsObject()
		parse, err := jsonObject.Get("parse")
		if err != nil || !parse.IsFunction() {
			gbase.CallRejectError(waiter.reject, gbase.NewTypeError(b.ctx, "JSON.parse not available"))
			return
		}
		if f, ok := parse.AsFunction(); ok {
			parsed, parseErr := f.Call([]engine.Value{engine.Str(string(b.data))})
			if parseErr != nil {
				gbase.CallRejectError(waiter.reject, gbase.NewTypeError(b.ctx, parseErr.Error()))
				return
			}
			gbase.CallResolve(waiter.resolve, parsed)
			return
		}
		gbase.CallRejectError(waiter.reject, gbase.NewTypeError(b.ctx, "JSON.parse failed"))
	default:
		gbase.CallResolve(waiter.resolve, engine.Str(string(b.data)))
	}
}
	...
	executor := engine.NewFunction("executor", func(eargs []engine.Value) (engine.Value, error) {
		...
		go func() {
			...
			if err != nil {
				ctx.PostTask(func() {
					defer release()
					gbase.CallRejectError(reject, gbase.NewTypeError(ctx, "fetch failed: "+err.Error()))
				})
				return
			}
			...
			if proxyURLStr != "" {
				...
				if proxyErr != nil || proxyURL.Scheme == "" || proxyURL.Host == "" {
					...
					ctx.PostTask(func() {
						defer release()
						gbase.CallRejectError(reject, gbase.NewTypeError(ctx, msg))
					})
					return
				}
