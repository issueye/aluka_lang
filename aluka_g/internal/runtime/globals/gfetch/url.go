...
// urlProto 是 URL 实例的原型（instanceof 支持）。
var urlProto engine.Object

// urlSearchParamsProto 是 URLSearchParams 实例的原型（instanceof 支持）。
var urlSearchParamsProto engine.Object

// NewURL 注册全局 URL / URLSearchParams 构造器。
func NewURL(ctx engine.Context, cfg URLConfig) error {
	urlCtor := engine.NewFunction("URL", func(args []engine.Value) (engine.Value, error) {
		if len(args) == 0 {
	...
	urlCtorObj, _ := urlCtor.AsObject()
	urlProto = engine.NewObject()
	_ = urlProto.Set("constructor", urlCtor)
	_ = urlCtorObj.Set("prototype", urlProto)

	spCtor := engine.NewFunction("URLSearchParams", func(args []engine.Value) (engine.Value, error) {
		var pairs []urlParam
	...
	spCtorObj, _ := spCtor.AsObject()
	urlSearchParamsProto = engine.NewObject()
	_ = urlSearchParamsProto.Set("constructor", spCtor)
	_ = spCtorObj.Set("prototype", urlSearchParamsProto)

	// URL.canParse(url[, base])：WHATWG 静态方法（Node ≥ 19.9）。
	_ = urlCtorObj.Set("canParse", engine.NewFunction("canParse", func(args []engine.Value) (engine.Value, error) {
		if len(args) == 0 {
	...
	// URL.createObjectURL(blob)/revokeObjectURL(url)：Node 22 提供但不注册
	// 可取回机制（blob: URL 无法 fetch），返回 `blob:nodedata:<uuid>` 形式。
	_ = urlCtorObj.Set("createObjectURL", engine.NewFunction("createObjectURL", func(args []engine.Value) (engine.Value, error) {
		return engine.Str("blob:nodedata:" + gbase.RandomUUID()), nil
	}))
	_ = urlCtorObj.Set("revokeObjectURL", engine.NewFunction("revokeObjectURL", func(args []engine.Value) (engine.Value, error) {
		return engine.Undefined(), nil
	}))

	_ = ctx.Global().Set("URL", urlCtor)
	_ = ctx.Global().Set("URLSearchParams", spCtor)
	return nil
}

// parseURL 解析 URL（支持 base 相对解析）。
func parseURL(input, base string) (*url.URL, error) {
	// Node/WHATWG 语义：空输入与含空白/控制字符的输入非法。
	if input == "" {
		return nil, fmt.Errorf("URL: invalid URL %q", input)
	}
...
func newURLSearchParamsInstance(pairs []urlParam, onChange func([]urlParam)) engine.Value {
	obj := engine.NewObject()
	if urlSearchParamsProto != nil {
		engine.SetProto(obj, urlSearchParamsProto)
	}
	...
	// set(name, value)：Node/WHATWG 语义——若键已存在则原地替换首次出现的
	// 条目并删除其余同名条目（保持位置）；否则追加到末尾。
	_ = obj.Set("set", engine.NewFunction("set", func(args []engine.Value) (engine.Value, error) {
		...
		if first < 0 {
			pairs = append(pairs, urlParam{key, val})
		} else {
			out := make([]urlParam, 0, len(pairs))
			for i, p := range pairs {
				if i == first {
	...
	// [Symbol.iterator]()：返回产出 [key, value] 对的迭代器（WHATWG 语义）。
	_ = obj.Set(engine.SymbolIterator.SymbolKey(), engine.NewFunction("[Symbol.iterator]", func(a []engine.Value) (engine.Value, error) {
		return newURLSearchParamsIterator(pairs), nil
	}))

	return obj
}

// newURLSearchParamsIterator 构造 URLSearchParams 迭代器（产出 [key, value] 对）。
func newURLSearchParamsIterator(pairs []urlParam) engine.Value {
	iterObj := engine.NewObject()
	idx := 0
	next := engine.NewFunction("next", func(a []engine.Value) (engine.Value, error) {
	...
	_ = iterObj.Set(engine.SymbolIterator.SymbolKey(), engine.NewFunction("[Symbol.iterator]", func(a []engine.Value) (engine.Value, error) {
		return iterObj, nil
	}))
	return iterObj
}
