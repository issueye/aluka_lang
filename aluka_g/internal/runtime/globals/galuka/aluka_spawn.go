...
func RegisterSpawn(ctx engine.Context, aluka engine.Value) {
	ao, _ := aluka.AsObject()
	...
	_ = ao.Set("spawnSync", engine.NewFunction("spawnSync", func(args []engine.Value) (engine.Value, error) {
		...
		_ = out.Set("stdout", gbuffer.NewBufferInstance(stdout.Bytes()))
		_ = out.Set("stderr", gbuffer.NewBufferInstance(stderr.Bytes()))
...
func alukaPipeStream(ctx engine.Context, r io.Reader) engine.Value {
	...
	stream, err := gstream.NewReadableStream(ctx, []engine.Value{engine.NewObjectFrom(map[string]engine.Value{
		...
		go func() {
			...
			if err != nil {
				if ee, ok := err.(*exec.ExitError); ok {
					...
					ctx.PostTask(func() {
						gbase.CallResolve(reject, engine.Str("Aluka.spawn: "+err.Error()))
					})
					return
				}
			}
			ctx.PostTask(func() {
				gbase.CallResolve(resolve, engine.IntValue(exitCode))
			})
		}()
		return engine.Undefined(), nil
	})
	p, _ := gbase.NewPromise(ctx, executor)
	return p
}
