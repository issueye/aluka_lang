...
func RegisterAlukaPassword(ctx engine.Context, aluka engine.Value) {
	ao, _ := aluka.AsObject()
	pw := engine.NewObject()
		...
		executor := engine.NewFunction("executor", func(ea []engine.Value) (engine.Value, error) {
			...
			go func() {
				...
				ctx.PostTask(func() {
					defer release()
					if err != nil {
						gbase.CallResolve(reject, engine.Str("Aluka.password.hash: "+err.Error()))
						return
					}
					gbase.CallResolve(resolve, engine.Str(encoded))
				})
			}()
			return engine.Undefined(), nil
		})
	...
	_ = pw.Set("verify", engine.NewFunction("verify", func(args []engine.Value) (engine.Value, error) {
		...
		executor := engine.NewFunction("executor", func(ea []engine.Value) (engine.Value, error) {
			...
			go func() {
				...
				ctx.PostTask(func() {
					defer release()
					gbase.CallResolve(resolve, engine.Boolean(ok))
				})
			}()
			return engine.Undefined(), nil
		})
...
func RegisterAlukaHash(ctx engine.Context, aluka engine.Value) {
	ao, _ := aluka.AsObject()
	hashFn := engine.NewFunction("hash", func(args []engine.Value) (engine.Value, error) {
		data := []byte("")
		if len(args) > 0 {
			data = gbase.ArgBytes(args, 0)
		}
		h := fnv.New64a()
	...
	_ = hashFo.Set("sha1", engine.NewFunction("sha1", func(args []engine.Value) (engine.Value, error) {
		sum := sha1.Sum(gbase.ArgBytes(args, 0))
