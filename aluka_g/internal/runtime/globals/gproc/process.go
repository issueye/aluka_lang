...
func NewProcess(ctx engine.Context, cfg ProcessConfig) error {
	...
	// platform / arch
	_ = proc.Set("platform", engine.Str(gbase.PlatformName()))
	_ = proc.Set("arch", engine.Str(gbase.ArchName()))
	...
	for {
		...
		if n > 0 {
			...
			ctx.PostTask(func() {
				...
				stdinMu.Unlock()
				chunk := engine.Value(gbuffer.NewBufferInstance(data))
				if encoding != "" {
