...
func consumeStream(ctx engine.Context, stream engine.Value, convert func([]engine.Value) engine.Value) (engine.Value, error) {
	...
	executor := engine.NewFunction("executor", func(args []engine.Value) (engine.Value, error) {
		...
		_, _ = nodeevents.CallEmitterMethod(stream, "on", []engine.Value{engine.Str("end"), endCb})
		_, _ = nodeevents.CallEmitterMethod(stream, "on", []engine.Value{engine.Str("error"), errCb})
		_, _ = nodeevents.CallEmitterMethod(stream, "on", []engine.Value{engine.Str("data"), dataCb})
		return engine.Undefined(), nil
	})
