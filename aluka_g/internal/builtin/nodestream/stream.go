...
func newReadableStream(ctx engine.Context, args []engine.Value) engine.Value {
	...
	_ = stream.Set("destroy", engine.NewFunction("destroy", func(args []engine.Value) (engine.Value, error) {
		...
		if len(args) > 0 && !args[0].IsUndefined() && !args[0].IsNull() {
			nodebase.EmitEvent(stream, "error", args[0])
		}
		state.ended = true
		state.buffer = nil
...
func installWritableMethods(stream engine.Object, state *writableState, args []engine.Value) {
	...
	_ = stream.Set("end", engine.NewFunction("end", func(args []engine.Value) (engine.Value, error) {
		...
		state.ended = true
		state.finished = true
		nodebase.EmitEvent(stream, "finish")
		nodebase.EmitEvent(stream, "close")
		return stream, nil
	}))
	...
	if transformFn != nil {
		_ = stream.Set("write", engine.NewFunction("write", func(args []engine.Value) (engine.Value, error) {
			...
			if f, ok := transformFn.AsFunction(); ok {
				...
				if err != nil {
					nodebase.EmitEvent(stream, "error")
					return engine.Boolean(false), nil
				}
...
func drainBuffer(state *streamState, event string) {
	for len(state.buffer) > 0 {
		...
		nodebase.EmitEvent(state.emitter, event, chunk)
	}
}
...
func finishReadable(state *streamState) {
	state.ended = true
	nodebase.EmitEvent(state.emitter, "end")
	nodebase.EmitEvent(state.emitter, "close")
}
