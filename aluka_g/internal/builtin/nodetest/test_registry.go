...
func NewTest(ctx engine.Context) (engine.Value, error) {
	...
	if av, err := nodeassert.NewAssert(ctx); err == nil {
		_ = m.Set("assert", av)
	}
	...
	_ = m.Set("run", engine.NewFunction("run", func(args []engine.Value) (engine.Value, error) {
		vm := nodebase.CurrentVM(ctx)
		if vm == nil {
			return engine.Undefined(), nil
		}
		TestProgrammaticRun = true
		stream := nodeevents.NewEmitterInstance()
		...
		ctx.PostTask(func() {
			...
			for _, r := range results {
				...
				_, _ = nodeevents.CallEmitterMethod(stream, "emit", []engine.Value{engine.Str("test:start"), data})
				...
				case r.Cancelled:
					cancelled++
					_, _ = nodeevents.CallEmitterMethod(stream, "emit", []engine.Value{engine.Str("test:fail"), d})
				case r.Skipped:
					skipped++
					_, _ = nodeevents.CallEmitterMethod(stream, "emit", []engine.Value{engine.Str("test:skip"), d})
				case r.Todo:
					todo++
					_, _ = nodeevents.CallEmitterMethod(stream, "emit", []engine.Value{engine.Str("test:todo"), d})
				case r.Passed:
					passing++
					_, _ = nodeevents.CallEmitterMethod(stream, "emit", []engine.Value{engine.Str("test:pass"), d})
				default:
					failing++
					if r.Error != "" {
			...
			_, _ = nodeevents.CallEmitterMethod(stream, "emit", []engine.Value{engine.Str("test:plan"), plan})
			_, _ = nodeevents.CallEmitterMethod(stream, "emit", []engine.Value{engine.Str("end")})
		})
		return stream, nil
	}))
