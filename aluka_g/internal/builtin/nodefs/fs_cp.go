...
func fsCpCopy(src, dest string, opts fsCpOptions, isTop bool) error {
	if isTop && opts.recursive {
		...
		if err1 == nil && err2 == nil {
			...
			if err == nil && rel != "." && rel != ".." && !isPathParent(rel) {
				return nodebase.NewFSCodeError("ERR_FS_CP_EINVAL", fmt.Sprintf("Cannot copy %s to a subdirectory of self %s", absSrc, absDest))
			}
		}
	}
	...
	if info.IsDir() {
		if !opts.recursive {
			...
			return nodebase.NewFSCodeError("ERR_FS_EISDIR", fmt.Sprintf("Recursive option not enabled, cannot copy a directory: %s", abs))
		}
	...
	if err == nil {
		if !opts.force {
			if opts.errorOnExist {
				return nodebase.NewFSCodeError("", fmt.Sprintf(", The file exists. %s", dest))
			}
