...
### 工作流硬约束（每次会话必须遵守，两条门禁缺一不可）

1. **开工先登记**：任何任务（含文档与流程改动）动手之前，必须先在**当日 TODO**
   （`.work/TODO/<YYYYMMDD>/README.md`）的 §2 待办清单里登记本次会话的任务列表
   （登记态为 `[ ]`，禁止预填完成态）。当日文件不存在则先按模板创建。
   会话中途新增任务同样先登记再动手——TODO 是开工门禁，不是收工补记。
2. **复审后才许提交**：任务完成后、`git commit` 之前，必须**复审实际改动内容**——
   逐块阅读 `git diff`（新文件读全文），核对三件事：改动与登记的 TODO 一致、
   无夹带无关改动、证据（测试/命令输出）真实可复现。复审通过才允许提交到本地；
   发现问题回改后重审。复审结论写进当日 TODO §3（复审方式 + 关键确认点）。
...
- **本地提交前置复审（硬门禁）**：提交前逐块阅读 `git diff --staged`（新文件
  读全文），确认「改动与当日 TODO 登记一致 / 无夹带无关改动 / 证据真实可复现」，
  复审结论记入当日 TODO §3——复审未通过不得提交。
- 提交前自查：
  ```bash
