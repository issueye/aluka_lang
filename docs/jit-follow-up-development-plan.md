...
| R4 | 部分完成 | 两路 PIC、有限内联、属性、push 和单一 upvalue 模式已落地；R4-1 调用约定扩展（0-4 参数数值叶子、Boolean 返回、多调用点、局部转发 callee 内联）与 R4-2 闭包扩展（多 numeric upvalue、只读捕获、非逃逸闭包）已完成（2026-08-12，见 §6.5 实施记录）；R4-3 至 R4-8 待后续批次 |
