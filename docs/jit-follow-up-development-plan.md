...
| R1-5 ✅ | 副作用提交协议 | 属性写、局部写、guarded 用户调用（数组 append/upvalue 直接写在通用 trace 中被 candidate 过滤挡在 trace 外，由 R4-2/R4-6 特化 matcher 复用本协议，见 §6.3 范围澄清）在 guard/deopt/异常/OOM/取消/safepoint 中断附近严格”只发生一次” | prepare/validate/commit 两阶段协议；verifier 显式拒绝非法副作用状态；固定回归与 jitdiff 事件日志证明无重复、无遗漏、无部分提交 |
...
**覆盖范围澄清**：通用 trace 编译路径（`candidate.go` 的 `rejectTraceCandidate`）的合法 op 白名单只含属性读写 + `OpStoreLocal` + 受保护 `OpCall`/`OpCallMethod` + `OpThrow`；直接的 `OpStoreUpvalue` 与数组 push 类 op 落入 `default` 被拒绝，全程由 Tier 0 兜底，不存在 JIT 内延迟提交路径。上表"数组 append"与"upvalue 写"两行描述的批量提交语义由 R4-6（`WriteNumberRange`/数组批处理）与 R4-2（numeric upvalue 原子写回）的专属 matcher 实现，复用本协议的 prepare→validate→commit 两阶段语义，但不走通用 candidate 路径——两者是"协议语义一致、接入路径不同"。
...
| 2026-08-12 | 字节码缓存版本 16→17（I-2，commit b08de6b）| 修复 const 绑定函数表达式的内联候选登记 bug：`compileFunction` 返回前未恢复 `lastFuncExprIdx`，函数体含嵌套函数表达式时内层编译覆盖该字段，导致调用点错误内联内层函数体。`compiler.go` 在 `compileFunction` 返回前恢复本函数模板索引；`serialize.go` `FormatVersion` 16→17 使含错误内联产物的旧磁盘缓存失效（"编译器语义变化时递增"的既有契约，延续 R2 阶段 OpInc/OpDec 的 15→16 bump）。新增 3 个嵌套箭头回归（IIFE/传参/filter，`inline_test.go`）。该轮未改默认 `--jit=off`、未扩 Native ABI |
...
| R3-3 ✅ | 宽松相等 | 抽取共享 primitive equality helper（`internal/engine/equality.go` `LooseEquals`/`StringToNumber`，Quick/Trace 复用）| 不在 JIT 复制另一套 ToPrimitive；对象一律按 identity guard；R4 集成期间完成（commit 9751113/c73ba65）|
...
### 10.4 R5-1/2/5/6/7 实施记录与 `--jit-stats` 读法

**R5-1 IR 优化 pass（常量折叠/冗余 store-load 消除/不可达块删除）**

`optimize.go` 的 `OptimizeProgram`（开关 `OptimizeIR = true`）三轮 pass：Pass 1 `foldConstOp` 常量折叠（覆盖 Add/Sub/Mul/Div/Mod/Pow，保留 IEEE-754 位语义 NaN/-0/±Inf/除零，且不跨分支目标）；Pass 2 相邻 `LOAD_LOCAL k / STORE_LOCAL k` 冗余消除（仅当非跳转目标）；Pass 3 `reachableFromEntry` 不可达块删除 + 跳转重映射，结果再次过 Verify。差分测试 `optimize_test.go`（`TestOptimizedLeafMatchesUnoptimizedExecution` 开关前后对 `{0,-0,3.5,NaN,Inf,-2.25}` 逐位比较，`TestOptimizeConstantFoldingBitSemantics`/`TestOptimizeDoesNotCrossBranchTargets`/`TestOptimizeEliminatesRedundantStoreLoad`/`TestOptimizeUnreachableBlockRemoval`）。

**R5-2 循环优化（保守 LICM，范围降级说明）**

§10.2 原始要求"纯数值 LICM、局部寄存器化"**未完整达成，降级关系如下**（commit `2977796` 已自述为 limitation，此处书面化）：没有独立 LICM/hoisting pass，也没有局部寄存器化。const-only 循环不变量由 R5-1 的常量折叠间接覆盖——`foldConstOp` 只接受纯 IEEE-754 算术，对调用/属性/异常类 op 一律 `return false`，故"只移动无调用/无属性写/无异常操作"的约束天然成立。非 const 的循环不变量（如 `x = a*2`，`a` 循环不变但非 const）的 hoisting 需要数据流分析，列为 limitation，留待后续。§4.1 启动状态表中"保守 LICM 范围（const-only 循环不变量折叠）"即指此降级范围。
