...
| 窗口：`show/hide/close/center/setTitle/setSize/getSize/setPosition/getPosition/setMinSize/setMaxSize/minimize/maximize/unmaximize/setAlwaysOnTop/setFullscreen/navigate/openDevTools/executeScript/on/emit` | ✅ | 同步控制为主 |
| `dialog.showMessageBox` / `showOpenDialog` | ⚠️ | Promise；选项字段远少于 `DialogOptions` |
...
| `dialog.*` | showOpenDialog / showSaveDialog / showMessageBox | ✅（主进程侧缺 save） |
...
| 问题 | 现象 | 影响 |
| :--- | :--- | :--- |
| **关闭拦截未贯通** | Go 侧有 `OnCloseRequested` / `RequestClose` 设计意图；前端 `close` 可走拦截，但主进程 JS `win.close()` 直接 `Close()`；原生 `WM_CLOSE` 路径也未统一走 `RequestClose` | 无法可靠实现「关窗口 → 藏托盘」；注释与行为不一致 |
| **对话框选项几乎未接线** | `DialogOptions` 含 `type/buttons/filters/defaultPath/directory/multiple` 等；JS 解析与 Win32 实现大多只用 `title/message`，消息框固定图标风格，open 不认多选/目录/过滤器（过滤器结构虽写了但主进程 JS 未传入） | 前端文档示例（`properties: ["openDirectory"]`）与真实行为不符 |
| **主进程缺 `showSaveDialog`** | 前端桥已有 `showSaveDialog`，`Aluka.gui.dialog` 没有对称 API | 同能力双入口不一致，主进程无法主动弹保存框 |
| **创建选项「声明了未解析」** | `WindowOptions` 有 `maximized` / `minimized` / `opacity` / `preloadScript`；`parseWindowOptions` 未读入后三项（及最大化/最小化）；preload 仅 darwin 路径部分使用 | 开发者按类型/注释传参会静默无效 |
| **工作区 WIP 未完成** | 本地未提交改动引入 `SetMenu(*Menu)` 但 `Menu` 类型不存在，且 `wsThickFrame` 作用域错误 → **当前工作树 `internal/gui` 无法通过 `go build`** | 合并前必须修完或回退；新 API 不要半落地 |

### 3.2 P1 — 能力已在 Go / 前端一侧，JS 表面缺失或不对称

| Go / 前端已有 | 主进程 JS 缺口 | 建议 |
| :--- | :--- | :--- |
| `IsMaximized` / `IsFullscreen` / `GetTitle` | 无对应查询 | 对齐前端：同步返回或统一 Promise |
| `SetResizable`（NativeWindow） | 无 `setResizable`；创建时 `resizable` 可读 | 补运行时 API |
| `SetHTML` | 无 `setHTML` | 与 `navigate` 成对暴露 |
| `toggleMaximize`（仅前端） | 主进程无 | 补一层薄封装即可 |
| `UnregisterRPCMethod` | 无 `unregisterRPC` | 热重载 / 插件场景需要 |
| `app.Windows` / `GetWindowByID` | 未暴露 | 多窗口管理刚需 |
| `events.off`（前端有） | 主进程 `on` 无取消 / 无返回 disposer | 易泄漏；Go `On` 已开始返回取消函数（WIP），JS 应对齐 |
| `shell` 仅路径类 | 无 `openExternal(url)` | 打开浏览器/mailto 是桌面标配 |

### 3.3 P2 — 产品完整度（对照 Electron / Tauri 的「常用子集」）

下列**不是**必须做成 Electron 1:1，但对「可发布应用」价值高：

1. **应用菜单栏**（窗口 `setMenu` / 全局 `Menu`）— 托盘菜单已有，窗口菜单尚未真正落地（`Menu` 类型缺失即信号）。  
...
| 项 | 现状 | 建议 |
| :--- | :--- | :--- |
| API 风格 | 设计稿 `new Window` / `Menu.buildFromTemplate`；实现为 `createWindow` / 菜单数组 | **冻结工厂风格为正式 API**；文档与 Demo 全面改写，避免双标准 |
...
| 关闭可取消 | ⚠️ 半成品 | ❌ | ⚠️ 部分路径 |
| Message / Open / Save 对话框 | ⚠️ 浅实现 | ⚠️ 无 Save | ✅ 三件套入口 |
| RPC | ✅ | ✅ register | ✅ call |
| 事件总线 | ✅ | ✅ on/emit | ✅ on/off/emit |
| 托盘 + 菜单 click | ✅ Win | ✅ | — |
| 全局快捷键 | ✅ Win | ✅ | — |
...
- [ ] 修复/完成或回退 WIP：`Menu` 类型、`wsThickFrame`、未接线的 `SetOpacity` / `SetProgressBar` / `SetOverlayIcon`  
- [ ] 统一关闭路径：`win.close()`、前端 close、系统关闭均 `RequestClose` → 可选取消；主进程暴露 `win.onCloseRequested(fn) => boolean` 或事件 `close` 的 `preventDefault` 风格（二选一，推荐 Electron 熟悉的事件对象）  
- [ ] `parseWindowOptions` 补齐：`maximized` / `minimized` / `opacity` / `preloadScript`（preload 需定义「字符串源码 vs 文件路径」语义）  
- [ ] 主进程补 `dialog.showSaveDialog`；三端对话框共用同一 options 解析器  
- [ ] 加深 Win32/Cocoa 对话框：至少 `filters`、`defaultPath`、`directory`；`multiple` 可第二刀  
- [ ] 回归：`demo/studio` + `aluka_gui_test` + gui 包 `go test`
...
仅列**建议冻结**的表面，便于评审；不等于已实现。

```ts
// aluka:gui（主进程）
export const app: {
  on(event: "ready" | "before-quit" | "quit", cb: (data?: unknown) => void): () => void;
  run(): void;
  quit(): void;
...
interface WindowHandle {
  readonly id: number;
  // 控制 + 查询 + on/off/emit + onCloseRequested / close 可取消
}
```
...
| 维度 | 判断 |
| :--- | :--- |
| 是否需要优化 | **是** — 关闭语义、对话框深度、选项解析、前后端对称、文档与实现统一 |
| 是否需要补充 | **是，但分阶段** — Phase B 补齐已有能力暴露；Phase C 按产品加菜单/通知/屏幕 |
| 当前能否做应用 | **能**（Windows 优先）— `demo/studio` 已覆盖 RPC/托盘/快捷键/无边框；缺口主要在「专业桌面体验」与「API 可信度」 |
| 最大风险 | 设计文档超前 + WIP 半成品入树 → 开发者按愿景编码失败；以及关闭/对话框静默错误行为 |

**建议的下一动作**：先完成 Phase A（含清理当前工作树编译错误），再落 Phase B 的 `.d.ts` 与对称 API；Phase C 起再开独立 milestone，并回写架构计划文档状态。
