...
| v1.7 | 2026-08-07 | **运行时监控器与内存上限（非兼容面新增能力）**：`--monitor[=interval]` 输出性能（指令数/速率、调用数、分配数、IC 命中率）、内存（HeapAlloc/Inuse/Sys/峰值、对象存活/累计）、运行时（goroutines、GC 次数/暂停）指标，text/json 两种格式，`--monitor-out` 落盘；`--max-memory=<bytes|KB|MB|GB>`（env `ALUKA_MAX_MEMORY`）进程内存上限——Go debug.SetMemoryLimit 软上限 + 看门狗（100ms 采样，超限先 GC 再判）→ VM 安全点（指令分发 + 对象分配 register 钩子）抛**可被 JS catch 捕获的 `RangeError: JavaScript heap out of memory`**（V8 同款；一次性消费标志使 catch 可运行）→ 连续超限约 0.5s 强制退出（码 3）防挂死。计数器 gated（常态零开销，实测解释器热循环性能影响 <1%）。新增 demo/monitor-demo.js 与单测（engine/monitor_test.go、monitor 包、cmd parseMemorySize、interpreter TestVMOOMLimit 端到端验证 catch 语义）。全量回归：go test 绿、差分 65/65、conformance 16/16。
