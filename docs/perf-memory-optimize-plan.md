...
|----|------|------|--------|---------|------|
| PF-1 | **调用快速路径** | `callClosure`/`invoke` 对小函数（无闭包捕获、参数少）走专用快速路径，跳过通用帧设置/upvalue 链 | P0 | ~300-500 行 | [x] |
     - **O-5（2026-08-07）**：跳过未引用 arguments 的函数每帧 arguments 对象创建（-52% 整体）。剩余：无闭包函数跳过帧初始化 |
...
| PF-3 | **字符串拼接合并** | `s += a + b + c` 编译为单次拼接指令（收集操作数一次 concat），消除中间分配 | P0 | ~150-300 行 | [~] |
     - 由 ME-1 rope 覆盖主要收益（strConcat 对 node 463x→3.1x）；编译期合并操作数作为后续候选 |
| PF-4 | **值栈预分配** | 编译期统计函数最大栈深，帧进入时一次性预留（消除 ensureStack 增量扩容） | P1 | ~150-250 行 | [x] |
     - 已核实：`callClosure` 已 `reserveUndefined(NumLocals)` 帧进入一次预留（O-3 结论） |
| PF-5 | **原生高阶内置** | `map`/`filter`/`reduce`/`forEach` 对 `[Symbol.iterator]` 数组走 Go 原生实现（免回调逐元素解释） | P1 | ~300-500 行 | [~] |
     - 循环已 Go 原生，但每元素 `fn.callWith` 仍走完整调用链；**O-6 回调原生化**（简单回调 Go 侧直执行）为 M-P1 最高优先级（docs/perf-optimization-plan.md） |
...
| ME-3 | **数组增长策略** | ArrayValue 追加按指数增长容量（当前可能逐元素 append 扩容）；稀疏数组转 map 存储 | P1 | ~200-350 行 | [x] |
     - arrayPush 实测 32x（对 node），指数扩容已生效（perf-compare arrayPush 525ms）；稀疏数组优化留后续 |
| ME-4 | **函数帧复用** | 帧对象池（嵌套调用复用已回收帧），降低 fib 类递归的帧分配 | P1 | ~250-400 行 | [ ] |
| ME-5 | **字符串 intern / 常量去重** | 模块常量池字符串去重（相同字面量共享）；属性名 intern 表 | P1 | ~150-300 行 | [~] |
     - **O-2 否定（2026-08-07）**：int→str intern 实测无提升（strConcat 瓶颈在 rope/平串创建）；常量池去重仍可作内存优化候选 |
...
基于 `docs/performance-report-v2.md`（评估）与池化专项评估的**实施方案**见
**docs/perf-optimization-plan.md**（v3）：O-6 回调原生化（M-P1 最高优先级）、
O-7 callIC 覆盖、O-4 isBigInt 合并、O-8 值宽值化（架构级）、O-9 rope 缓存、
O-10 JIT 评估；含里程碑/验收/风险/复现。已排除：args 切片池、int→str intern
（实测无提升）。
