...
| ID | 任务 | 说明 | 优先级 | 工作量估 | 状态 |
|----|------|------|--------|---------|------|
| ME-1 | **字符串 rope / 延迟拼接** | `s += ...` 在 VM 侧维护分段串（rope 节点），`String()`/比较/索引时才展平；消除 O(n²) 累积复制 | P0 | ~400-600 行 | [ ] |
| ME-2 | **对象字面量批量分配** | `{a,b,c}` 编译期预知 shape：一次分配 slots 容量 + 批量写入（跳过逐属性 Set/transition） | P0 | ~200-350 行 | [ ] |
...
| ME-7 | **GC 触发调优** | 分配阈值/清扫周期参数化（当前固定 gcSweepEvery=4096）；`--mem` 报告峰值 | P2 | ~100-200 行 | [ ] |
