...
| v1.7 | 2026-08-06 | **M5 收尾**（commit 99e24cd）：TypeScript `declare` 环境声明擦除、包 `exports` 字段解析（条件导出/子路径）、多项运行时补全；差分框架扩展至 15/15（新增 15-test-runner.cjs——`//@test` 标记触发 run.sh 用 `node --test`/`aluka test` 归一化对比，覆盖 before/after/skip/todo/t.plan/子测试/注册顺序） |
