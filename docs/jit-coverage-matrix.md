...
| R2 硬停止条件：Native Frame ABI 无 Go 指针槽位（reflect 递归验证全部字段为数值/数值数组类型；结论仅覆盖 Frame 结构本身） | `TestFrameIsPointerFree`（`frame_pointer_free_test.go`） | 全部平台 |
...
| R1-7 fuzz：`CompileTraceWithGuards`（非对齐 start/backedge、malformed bytecode、无效 call/method guard、unsupported opcode、属性写/throw/jump-fixup 组合；code 截断至 256 字节；仅静态确认所有环都经过 budgeted jump-to-zero 的产物以 budget=4 执行） | `FuzzCompileTrace`、`TestFuzzTraceExecutionBounded`（jit 包；seed 含 throwTraceTemplate/sideEffectTraceTemplate 编码） |
| R1-7 fuzz：deopt/exception 恢复（随机 DeoptExit 字段、ResumePC、LocalSlots、StackValues、PendingException nil/Number/NaN/String/Object；正常出口在恢复栈前校验非 nil 模板、完整指令流及 PC 非负/对齐/界内；失败必须返回错误不 panic、不提交 VM 状态） | `FuzzResumeTraceExit`、`TestResumeTraceExitRejectsInvalidResumePC`（interpreter 包；VM 每输入新建） |
...
| R1-7 fuzz：artifact replay（随机/变异 case/meta/IR；SaveArtifact/LoadArtifact/Replay 错误可控；seed/source/case ID 精确往返且零值合法；body 池为 R1-6 mutation 固定用例的有界程序） | `FuzzArtifactReplay`（jitdiff 包） |
