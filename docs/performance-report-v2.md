...
## 11. 池化专项评估（2026-08-07，实测否定/确认）

用户提问"部分点位使用池化是否有提升"——对三个候选点位做了实测：

| 池化点位 | 实验 | 结果 |
|---------|------|------|
| 调用参数切片池（doCall/doCallMethod/doNew/doCallThis 的 `make([]engine.Value,n)`） | VM 级 LIFO 复用 + defer 归还（6 处） | **无提升**（调用负载 1103-1130ms vs 基线 1097-1147ms，噪声内）。原因：小切片多被 Go 逃逸分析栈分配，池化自身分支/defer 开销抵消；pprof 中该行主要是 copy 而非分配 |
| int→字符串 intern 表（strConcat 的 `'x'+i` 数字转换，sync.Map 缓存 -10000..10000） | formatNumber 查表 | **无提升**（48.4-51.9ms vs 基线 48.4-50.4ms）。原因：strConcat 瓶颈在 rope 节点/平串创建，数字串转换后立即被合并，intern 不减少创建 |
| 值栈预分配（PF-4） | 核实现状：callClosure 已 `reserveUndefined(NumLocals)` 帧进入一次预留 | **已实现**（无需再池化） |

### 为什么显式池化对 JS 运行时收益有限

1. **JS 值生命周期由 GC 管理**：rope 节点、upvalue、对象、闭包被 JS 引用，无法显式复用
   （V8 用分代 GC 而非对象池的原因）；显式池化需弱引用追踪，复杂度/风险高。
2. **Go 逃逸分析已覆盖简单临时值**：栈上分配的小切片/标量无堆成本，池化只加开销。
3. **有效路径是"减少创建"而非"复用"**：
   - O-5 跳过未引用 arguments 的对象创建（已实施，-52%）；
   - O-6 数组高阶回调 Go 侧直调（待实施）——"不建帧"；
   - open upvalue 共享已实现（同槽多闭包复用）。
4. 可池化的纯 VM 内部结构（vmTryHandler 等）占比 <1%，收益微。

**结论**：池化不是当前主要优化方向；继续沿"减少创建 + 原生直调"（O-6→O-3→O-4→O-8）推进。
