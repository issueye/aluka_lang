...
func NewEncoding(ctx engine.Context, cfg EncodingConfig) error {
	...
	atob := func(args []engine.Value) (engine.Value, error) {
		...
		b, err := gbuffer.DecodeBase64(args[0].String())
		if err != nil {
			return engine.Undefined(), gbase.ThrowDOMException(ctx, "InvalidCharacterError", "Invalid character")
		}
	...
	btoa := func(args []engine.Value) (engine.Value, error) {
		...
		for _, r := range s {
			if r > 0xFF {
				return engine.Undefined(), gbase.ThrowDOMException(ctx, "InvalidCharacterError", "The string contains characters outside of the Latin1 range")
			}
...
func newTextEncoderStream(ctx engine.Context) engine.Value {
			...
			if c, ok := a[1].AsObject(); ok {
				if e, err := c.Get("enqueue"); err == nil && e.IsFunction() {
					if f, ok := e.AsFunction(); ok {
						if _, err := f.Call([]engine.Value{gbuffer.NewInstance([]byte(s))}); err != nil {
	...
	ts := gstream.NewTransformStream(ctx, []engine.Value{engine.NewObjectFrom(map[string]engine.Value{"transform": transform})})
...
func newTextDecoderStream(ctx engine.Context, args []engine.Value) engine.Value {
	...
	transform := engine.NewFunction("transform", func(a []engine.Value) (engine.Value, error) {
		var decoded string
		if len(a) > 0 {
			if data, ok := gbase.BytesOf(a[0]); ok {
				decoded = gbuffer.Decode(stripBOM(data, enc), enc)
			}
		}
		if len(a) > 1 {
			if c, ok := a[1].AsObject(); ok {
...
func newTextEncoder() engine.Value {
	...
	_ = enc.Set("encode", engine.NewFunction("encode", func(args []engine.Value) (engine.Value, error) {
		...
		return gbuffer.NewInstance([]byte(s)), nil
	}))
