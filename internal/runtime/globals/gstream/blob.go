...
func newBlobInstance(ctx engine.Context, args []engine.Value, typeHint string) engine.Value {
						...
						if e, err := c.Get("enqueue"); err == nil && e.IsFunction() {
							if f, ok := e.AsFunction(); ok {
								for _, ch := range chunks {
									if _, err := f.Call([]engine.Value{gbuffer.NewBufferInstance(ch)}); err != nil {
	...
	_ = blob.Set("slice", engine.NewFunction("slice", func(a []engine.Value) (engine.Value, error) {
		...
		start := 0
		if len(a) > 0 {
			start = gbase.ArgInt(a, 0, 0)
		}
		end := n
		if len(a) > 1 {
			end = gbase.ArgInt(a, 1, n)
		}
		if start < 0 {
			start = n + start
		}
		...
		start = gbase.ClampIdx(start, 0, n)
		end = gbase.ClampIdx(end, start, n)
		sliceType := blobType
		if len(a) > 2 {
		...
		sub := data[start:end]
		// 构造新 Blob（用原始字节直接包装）。
		nb := newBlobInstance(ctx, []engine.Value{gbuffer.NewBufferInstance(sub)}, sliceType)
		return nb, nil
	}))
