...
func NewEvent(ctx engine.Context, cfg EventConfig) error {
	...
	EnsureTargetProto()
	engine.SetProto(TargetProto, ctx.ObjectPrototype())
...
// 返回 !defaultPrevented。
func Dispatch(target engine.Value, ev engine.Value) bool {
	state := eventTargetStateOf(target)
	if evObj, ok := ev.AsObject(); ok {
...
func NewEventTargetInstance() engine.Value {
	EnsureTargetProto()
	et := engine.NewObject()
	engine.SetProto(et, TargetProto)
