...
func RegisterUtil(ctx engine.Context, aluka engine.Value) {
	ao, _ := aluka.AsObject()
		...
		executor := engine.NewFunction("executor", func(ea []engine.Value) (engine.Value, error) {
			...
			go func() {
				...
				ctx.PostTask(func() {
					defer release()
					if err != nil {
						gbase.CallResolve(reject, engine.Str("Aluka.dns.lookup: "+err.Error()))
						return
					}
					if len(addrs) == 0 {
						gbase.CallResolve(reject, engine.Str("Aluka.dns.lookup: no addresses"))
						return
					}
					gbase.CallResolve(resolve, engine.Str(addrs[0]))
				})
			}()
			return engine.Undefined(), nil
		})
