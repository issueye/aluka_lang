...
func RegisterCompress(ctx engine.Context, aluka engine.Value) {
	ao, _ := aluka.AsObject()

	_ = ao.Set("deflateSync", engine.NewFunction("deflateSync", func(args []engine.Value) (engine.Value, error) {
		var buf bytes.Buffer
		w := zlib.NewWriter(&buf)
		if _, err := w.Write(gbase.ArgBytes(args, 0)); err != nil {
			return engine.Undefined(), err
		}
		if err := w.Close(); err != nil {
			return engine.Undefined(), err
		}
		return gbuffer.NewBufferInstance(buf.Bytes()), nil
	}))
		...
		executor := engine.NewFunction("executor", func(ea []engine.Value) (engine.Value, error) {
			...
			go func() {
				...
				ctx.PostTask(func() {
					defer release()
					if err != nil {
						gbase.CallResolve(reject, engine.Str(err.Error()))
						return
					}
					gbase.CallResolve(resolve, gbuffer.NewBufferInstance(out))
				})
			}()
			return engine.Undefined(), nil
		})
