...
func newBufferExports() engine.Value {
	ctor := engine.NewFunction("Buffer", func(args []engine.Value) (engine.Value, error) {
		...
		if arg.Type() != engine.TypeBoolean && !arg.IsObject() && !arg.IsFunction() {
			if n, ok := arg.Int(); ok {
				...
				return newBufferInstance(make([]byte, n)), nil
			}
		}
	...
	_ = co.Set("allocUnsafe", engine.NewFunction("allocUnsafe", func(args []engine.Value) (engine.Value, error) {
		...
		return newBufferInstance(make([]byte, size)), nil
	}))
...
func newBufferInstance(data []byte) engine.Object {
	...
	_ = buf.Set("toString", engine.NewFunction("toString", func(args []engine.Value) (engine.Value, error) {
		...
		if len(args) > 2 {
			end = argInt(args, 2, len(d))
		}
		start = clampIdx(start, 0, len(d))
		end = clampIdx(end, start, len(d))
		return engine.Str(decodeBuffer(d[start:end], encoding)), nil
	}))
	...
	_ = buf.Set("write", engine.NewFunction("write", func(args []engine.Value) (engine.Value, error) {
		...
		for i := 1; i < len(args); i++ {
			...
			default:
				if i == 1 {
					offset = argInt(args, i, 0)
				} else {
					if requested := argInt(args, i, -1); requested >= 0 && requested < maxLen {
						maxLen = requested
					}
				}
			}
		}
		offset = clampIdx(offset, 0, len(d))
	...
	sliceFn := engine.NewFunction("slice", func(args []engine.Value) (engine.Value, error) {
		...
		if len(args) > 1 {
			end = argInt(args, 1, len(d))
		}
		start = clampIdx(start, 0, len(d))
		end = clampIdx(end, start, len(d))
		return newBufferInstance(d[start:end]), nil
	})
	...
	_ = buf.Set("copy", engine.NewFunction("copy", func(args []engine.Value) (engine.Value, error) {
		...
		targetStart := argInt(args, 1, 0)
		sourceStart := argInt(args, 2, 0)
		sourceEnd := argInt(args, 3, len(d))
		sourceStart = clampIdx(sourceStart, 0, len(d))
		sourceEnd = clampIdx(sourceEnd, sourceStart, len(d))
...
func bufferFromArgs(args []engine.Value) (engine.Value, error) {
	...
	case val.Type() == engine.TypeString:
		return newBufferInstance(encodeBuffer(val.String(), encoding)), nil
...
func decodeBuffer(data []byte, encoding string) string {
	enc, ok := normalizeEncoding(encoding)
	if !ok {
		enc = "utf8"
	}
