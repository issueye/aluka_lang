...
func (p *Pattern) isFirst() bool { return p.indexes[0] }

func (p *Pattern) isLast(isDir bool) bool {
	last := len(p.parts) - 1
	if p.indexes[last] {
		return true
	}
...
func (p *Pattern) test(idx int, name string) bool {
	if idx >= len(p.parts) {
		return false
	}
	s := p.parts[idx]
...
func (p *Pattern) child(indexes map[int]bool) *Pattern {
	return &Pattern{parts: p.parts, strs: p.strs, indexes: indexes, nocase: p.nocase}
}

// cacheKey 对应 Pattern.cacheKey：strs[index] 起以 '/' 连接。
func (p *Pattern) cacheKey(index int) string {
...
func CompilePattern(pattern string, nocase bool) []*Pattern {
	...
	for _, e := range expanded {
		...
		out = append(out, &Pattern{parts: parts, strs: strs, indexes: indexes, nocase: nocase})
	}
	return out
}
...
func NewEngine(root string, nocase bool, excludeFn func(string) bool) *Engine {
	return &Engine{
		root:       root,
		nocase:     nocase,
...
func (g *Engine) cacheAdd(path string, p *Pattern) bool {
	set, ok := g.seen[path]
	if !ok {
		set = map[string]bool{}
		g.seen[path] = set
...
func (g *Engine) seenAt(path string, p *Pattern, index int) bool {
	set, ok := g.seen[path]
	if !ok {
		return false
	}
...
func (g *Engine) addResult(value string) {
...
func (g *Engine) addSubpattern(path string, p *Pattern) {
	if i, ok := g.subIdx[path]; ok {
		g.subs[i].patterns = append(g.subs[i].patterns, p)
	} else {
		g.subIdx[path] = len(g.subs)
		g.subs = append(g.subs, globSub{path: path, patterns: []*Pattern{p}})
	}
}

func (g *Engine) statSync(path string) *globStat {
...
func (g *Engine) readdirSync(path string) []globDirent {
...
func (g *Engine) addSubpatterns(path string, p *Pattern) {
...
func (p *Pattern) hasSeenSymlinks() bool {
	return true
}

// globSyncRun 主循环（对应 Glob.globSync）。
func (g *Engine) SyncRun(patterns []*Pattern) []string {
...
func ToResults(ge *Engine, paths []string, withFileTypes bool, root string) []engine.Value {
...
// Node 22.23 实测不生效）。
func ParseOptions(opts engine.Value) (root string, excludeFn func(string) bool, withFileTypes bool, err error) {
	root = "."
	if opts.IsUndefined() {
		return
	}
	...
	if v, e := o.Get("exclude"); e == nil && !v.IsUndefined() {
		if f, ok2 := v.AsFunction(); ok2 {
			...
			for _, k := range arr.Keys() {
				...
				patterns = append(patterns, CompilePattern(strings.ReplaceAll(absPat, "\\", "/"), nocase)...)
			}
			excludeFn = func(path string) bool {
...
func globFullMatch(p *Pattern, path string) bool {
	// 路径转 '/' 分段（Windows 反斜杠）。
	path = strings.ReplaceAll(path, "\\", "/")
	return FullMatchExact(p, path)
}

// globFullMatchExact 全路径匹配（不做反斜杠归一化——posix 语义下
// '\' 是字面字符，用于 path.matchesGlob）。绝对路径 pattern 的首段 ''
// 与路径首段 '' 直接对齐匹配。
func FullMatchExact(p *Pattern, path string) bool {
