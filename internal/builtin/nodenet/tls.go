		...
		go func() {
			...
			for {
				...
				go func(c net.Conn) {
					...
					socket, _ := NewSocket(ctx, c)
					ctx.PostTask(func() {
						nodebase.EmitEvent(server, "secureConnection", socket)
...
func tlsConnect(ctx engine.Context, args []engine.Value) engine.Value {
	...
	socket, state := NewSocket(ctx, nil)
	go func() {
		...
		if err := tconn.Handshake(); err != nil {
			_ = raw.Close()
			ctx.PostTask(func() {
				nodebase.EmitEvent(socket, "error", engine.Str("tls: handshake failed: "+err.Error()))
				state.close()
			})
			return
		}
		...
		ctx.PostTask(func() {
			...
			nodebase.EmitEvent(socket, "secureConnect")
		})
