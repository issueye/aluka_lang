		...
		if len(args) > 0 && args[0].IsObject() {
			if o, ok := args[0].AsObject(); ok {
				if c, err := o.Get("categories"); err == nil {
					...
					if !ok {
						return engine.Undefined(), &codedError{err: fmt.Errorf("%w: The \"options.categories\" property must be an instance of Array. Received type %s", engine.ErrTypeError, c.Type()), code: "ERR_INVALID_ARG_TYPE"}
					}
					if len(arr.Elems()) == 0 {
						return engine.Undefined(), &codedError{err: fmt.Errorf("%w: At least one category is required", engine.ErrTypeError), code: "ERR_TRACE_EVENTS_CATEGORY_REQUIRED"}
					}
